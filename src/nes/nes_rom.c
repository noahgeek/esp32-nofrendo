...
/* ===== ESP32 partition mmap support =====
 * 当 filename 以 '@' 开头时，将该名字作为分区标签，使用 esp_partition_mmap
 * 把 ROM 直接映射到 Flash 地址空间，避免占用大量 DRAM。
 */
#ifdef ESP_PLATFORM
#include "esp_partition.h"
#include "esp_log.h"
static spi_flash_mmap_handle_t s_rom_mmap_handle = 0;
static const void *s_rom_mmap_ptr = NULL;
#endif
...
#ifdef ESP_PLATFORM
   /* === mmap 分支：filename 以 '@' 开头表示从指定分区 mmap === */
   if (filename && filename[0] == '@')
   {
      const char *part_label = filename + 1; /* 去掉 '@' 前缀 */
      /* 使用 "Default 4MB with spiffs" 分区方案，其中 "spiffs" 分区
       * 位于 0x290000，大小 0x160000。我们把 ROM 直接烧录到这个分区，
       * 然后通过 esp_partition_mmap 映射到 CPU 地址空间。 */
      const esp_partition_t *part = esp_partition_find_first(
          ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_SPIFFS, part_label);
      if (NULL == part)
      {
         nofrendo_log_printf("rom_load: partition '%s' not found\n", part_label);
         goto _fail;
      }
      ...
      if (err != ESP_OK)
      {
         nofrendo_log_printf("rom_load: esp_partition_mmap failed (%d)\n", err);
         goto _fail;
      }

      const uint8 *rom_base = (const uint8 *)s_rom_mmap_ptr;

      /* 校验 iNES magic */
      if (memcmp(rom_base, ROM_INES_MAGIC, 4) != 0)
      {
         nofrendo_log_printf("rom_load: not a valid iNES image in partition\n");
         goto _fail;
      }

      /* 解析 header（格式与 inesheader_t 一致） */
      strncpy(rominfo->filename, filename, PATH_MAX);
      rominfo->rom_banks  = rom_base[4];
      rominfo->vrom_banks = rom_base[5];
      uint8 rom_type      = rom_base[6];
      uint8 mapper_hi     = rom_base[7];
      rominfo->sram_banks = 8;
      rominfo->vram_banks = 1;
      rominfo->mirror = (rom_type & ROM_MIRRORTYPE) ? MIRROR_VERT : MIRROR_HORIZ;
      rominfo->flags = 0;
      if (rom_type & ROM_BATTERY)    rominfo->flags |= ROM_FLAG_BATTERY;
      if (rom_type & ROM_TRAINER)    rominfo->flags |= ROM_FLAG_TRAINER;
      if (rom_type & ROM_FOURSCREEN) rominfo->flags |= ROM_FLAG_FOURSCREEN;
      rominfo->mapper_number = (rom_type >> 4) | (mapper_hi & 0xF0);
      ...
      /* ROM/VROM 直接指向 Flash mmap 区域，零拷贝 */
      const uint8 *p = rom_base + sizeof(inesheader_t);
      if (rominfo->flags & ROM_FLAG_TRAINER)
      {
         memcpy(rominfo->sram + TRAINER_OFFSET, p, TRAINER_LENGTH);
         p += TRAINER_LENGTH;
      }
      rominfo->rom = (uint8 *)p;  /* mmap 数据是只读的，但 NES ROM 本就只读 */
      p += rominfo->rom_banks * ROM_BANK_LENGTH;

      if (rominfo->vrom_banks)
      {
         rominfo->vrom = (uint8 *)p;
      }
      else
      {
         rominfo->vram = NOFRENDO_MALLOC(VRAM_LENGTH);
         if (NULL == rominfo->vram) goto _fail;
         memset(rominfo->vram, 0, VRAM_LENGTH);
      }
      ...
      nofrendo_log_printf("ROM mmap'd from partition '%s': %dKB PRG, %dKB CHR, mapper %d\n",
                          part_label,
...
#ifdef ESP_PLATFORM
   /* 如果 ROM 来自 mmap，则 rom/vrom 指针指向 Flash 区域，不能 free */
   if (s_rom_mmap_handle)
   {
      spi_flash_munmap(s_rom_mmap_handle);
      s_rom_mmap_handle = 0;
      s_rom_mmap_ptr = NULL;
   }
   else
   {
      if ((*rominfo)->rom)  NOFRENDO_FREE((*rominfo)->rom);
      if ((*rominfo)->vrom) NOFRENDO_FREE((*rominfo)->vrom);
   }
#else
   if ((*rominfo)->rom)
