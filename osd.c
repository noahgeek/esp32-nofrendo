...
#include "src/noftypes.h"

#include "src/event.h"
#include "src/gui.h"
#include "src/log.h"
#include "src/nes/nes.h"
#include "src/nes/nes_pal.h"
#include "src/nes/nesinput.h"
#include "src/nofconfig.h"
#include "src/osd.h"
...
/* 暂停/恢复 nofrendo 定时器，给 BLE 连接留出资源 */
void nofrendo_pause_timer() {
	if (timer) xTimerStop(timer, 0);
}

void nofrendo_resume_timer() {
...
extern void *mem_alloc(int size, bool prefer_fast_memory)
{
	...
	if (!ptr)
	{
		nofrendo_log_printf("mem_alloc FAILED: size=%d, free_heap=%d\n", size, esp_get_free_heap_size());
	}
	return ptr;
}
...
int osd_init()
{
	...
	// ESP32-C3 single core: larger stack, higher priority
